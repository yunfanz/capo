...
class OppSolver:
    ...
    time series'''
    #@profile
    def __init__(self, tracking=False, fqs=np.linspace(.14,.16,20), T1=np.arange(2456681.4, 2456681.6, 0.001), cal='psa6622_v003', beam='PAPER', bandpass=None):
    ...
    def prepare_coord(self):
        ...
        m0 = np.linalg.inv(self.aa.eq2top_m)
        ex,ey,ez = np.dot(m0, self.top0)
    ...
    def opp(self, bl1=None,bl2=None, bl1coords=None, bl2coords=None, sky=False, rephase=0, delay=False, return_series=False, debug=False):
        ...
        if debug:
            ...
            V2 = self.bms * self.sky * np.exp(self.k*bl2_prj).transpose((1,0,2)) #(time, freq, space)
            V1 = self.bms * self.sky * np.exp(self.k*bl1_prj).transpose((1,0,2))
            ...
            if delay:
                res = np.fft.fftshift(np.fft.ifft(
                    np.sum(np.mean(
                        np.fft.fft(self.bms * self.sky * np.exp(self.k*bl2_prj).transpose((1,0,2)),axis=0)*np.conj(np.fft.fft(self.bms* self.sky * np.exp(self.k*bl1_prj).transpose((1,0,2)),axis=0)),axis=2), axis=1, keepdims=True),axis=0), axes=0)
        
            else: #return results as function of frequency
                res = np.fft.fftshift(np.fft.ifft(
                    np.mean(
                        np.fft.fft(self.bms * self.sky* np.exp(self.k*bl2_prj).transpose((1,0,2)),axis=0)*np.conj(np.fft.fft(self.bms* self.sky * np.exp(self.k*bl1_prj).transpose((1,0,2)),axis=0)),axis=2),axis=0), axes=0)
